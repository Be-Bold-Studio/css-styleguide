...
Перед каждой группой свойств добавляй комментарий с названием этой группы: `Positioning`, `Box model`, `Border`, `Typography`, `Visual` и `Misc`.

В рамках групп `Positioning` и `Box model` объявления сортируются логически (сначала более влиятельные, затем менее). В рамках остальных групп в алфавитном порядке. Например, задавая блоку отображение в виде flex-контейнера, следует сначала написать `display: flex`, а уже затем объявить другие flex-свойства.
...
При написании стилей на SCSS следует использовать однострочные комментари, в формате `// Comment`. Т.к. такие комментарии удаляются при компиляции стилей вне зависимости от настроек.
...
#### Пример
Примерно вот какой логике должен следовать разработчик при именовании блока.
...
4. Мы используем БЭМ, поэтому можно часть названия с блоком пропустить. Получаем `.promotion`.
5. Сокращаем по возможности. В данном случае есть общепринятое сокращение. И получаем идеальный вариант — `.promo`.

### Примечание для CSS- и SCSS-модулей в React

Каждый (S)CSS-модуль использует свою область видимости. Это позволяет разработчикам быть более свободными в именовании классов. Например, не нужно заботиться об уникальности имени класса во всём проекте и можно на использовать одинаковые имена в разных модулях.

Также, поскольку в рамках компонента, все вложенные элементы по сути являются БЭМ-элементами, пропадает необходимость использования БЭМ-нейминга. Все остальные описанные в этой секции правила, следует применять.
